...
onMounted(() => {
  ...
  let deviceCartesian
  function initEvent() {
    new Cesium.ScreenSpaceEventHandler(viewer.canvas).setInputAction(e => {
  ...
  //只有设备才有监控
  watch(
    () => props.mapTreeChecked,
    newVal => {
      let aLength = newVal.length
      let bLength = storageTree.value.length
      if (bLength > aLength) {
        let diffTree = storageTree.value.filter(
          obj => !newVal.includes(obj) && obj.children == undefined
        )
        if (diffTree && diffTree.length) {
          for (let i = 0; i < diffTree.length; i++) {
            ...
            if (diffTree[i].category == 1) {
              ...
              getScreenSiteById({ objectCategoryId: diffTree[i].key }).then(
                res => {
                  if (res?.data?.length) {
                    for (let i = 0; i < res.data.length; i++) {
                      viewer.entities.removeById(res.data[i].id)
              ...
              for (let i = 0; i < videoMonitorPoint.length; i++) {
                viewer.entities.removeById(
                  videoMonitorPoint[i].channel.toString()
                )
              }
              for (let i = 0; i < allPrimitives.length; i++) {
                for (let j = 0; j < monitorVideoList.value.length; j++) {
                  if (
                    Number(monitorVideoList.value[j].channel) ==
                    Number(allPrimitives[i].id)
                  ) {
  ...
  getLand().then(async res => {
    if (res?.data?.length) {
      for (let i = 0; i < res.data.length; i++) {
        let h = 11
        if (
          res.data[i].name.indexOf('大棚') != -1 ||
          res.data[i].name == '测坑测桶试区'
        ) {
          h = 15
        }
        viewer.entities.add({
          ...
          label: {
            text: res.data[i].landArea
              ? res.data[i].name + '\n' + res.data[i].landArea + '亩'
              : res.data[i].name,
